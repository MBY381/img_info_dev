...
def top_matrix_compute(filename="top_matrix.txt"):
    ...
    top_image = cv2.imread(IMAGES_FOLDER + "top_undistorted.jpg")
    # 求取透视变换矩阵
...
def left_matrix_compute(filename="left_matrix.txt"):
    ...
    if os.path.isfile(MATRIX_FOLDER + filename):
        os.remove(MATRIX_FOLDER + filename)
    left_image = cv2.imread(IMAGES_FOLDER + "left_undistorted.jpg")
    # 求取透视变换矩阵
    # left透视变换矩阵
    ...
    for j in range(0, LEFT_BLOCKS[1], 1):
        ...
        if j == 0:
            ...
            # cv2.imwrite(OUT_IMAGES_FOLDER + f"test/LEFT_WRAP_{j}.jpg", LEFT_IMAGE)
            # cv2.waitKey(0)
...
def right_matrix_compute(filename="right_matrix.txt"):
    ...
    if os.path.isfile(MATRIX_FOLDER + filename):
        os.remove(MATRIX_FOLDER + filename)
    right_image = cv2.imread(IMAGES_FOLDER + "right_undistorted.jpg")
    # 求取透视变换矩阵
...
def bottom_left_matrix_compute(filename="bottom_left_matrix.txt"):
    ...
    if os.path.isfile(MATRIX_FOLDER + filename):
        os.remove(MATRIX_FOLDER + filename)
    bottom_left_image = cv2.imread(IMAGES_FOLDER + "bottom_left_undistorted.jpg")
    # 求取透视变换矩阵
...
def bottom_right_matrix_compute(filename="bottom_right_matrix.txt"):
    ...
    if os.path.isfile(MATRIX_FOLDER + filename):
        os.remove(MATRIX_FOLDER + filename)
    bottom_right_image = cv2.imread(IMAGES_FOLDER + "bottom_right_undistorted.jpg")
    # 求取透视变换矩阵
...
if __name__ == '__main__':
    ...
    cv2.imwrite(OUT_IMAGES_FOLDER + "FRAME.jpg", FRAME)
    cv2.waitKey(0)
